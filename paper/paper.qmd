...
The independent variable "Median Income" is a continuous variable representing the median income of all immigrants in each Admission Category. The dependent variable "Total Count" is the total number of immigrants granted citizenship in each Admission Category.


## Model Justification

In our analysis, we aimed to estimate the relationship between admission categories and the total number of immigrants, considering the overdispersion observed in the data. We initially considered employing a Negative Binomial regression model due to its suitability for count data, and categorical independent variables, which aligns with our data, as seen in @tbl-table4. However, we noticed that both Poisson and Negative Binomial regression models yielded the same coefficient estimates, suggesting that both models are equally appropriate for the data. Therefore, we decided to use the Poisson regression model for our analysis.
<!--
...
More interesting findings are observed in @fig-fig3 is an illustration of Median Income (in $CAD) by Admission Category from 2001 to 2016. The graph shows a strong positive coorelation between the year and the median income, as the median income increases over the years. This is expected as the economy grows, the median income of immigrants also increases. The graph also shows the trend line for each admission category, which indicates the direction of the trend. Using this line we observe that `Economic Immigrant, principal applicant` has the highest median income among all the admission categories reached \$30,000 CAD, even bigger than the `Total, immigrant admission cateogry` suggesting that principal applicant is way above the average. It is almost increasing linearly over the years, suggesting Canad's policies of attracting high income immigrants to the country.
...
By utilizing this model both of these models we can estimate wheather a new immigrant from a certain admission category will be granted citizenship based on their income level, essentially the core goal of our analysis.

# Discussion {#sec-discussion}

In this paper, we analysed the trends between the citizenship rate among various Immigrant admission category and their income levels in Canada from 2001 to 2016. In addition, to that we also examined the income distribution among admission categories in 2021, allowing us to understand the correlation between these varaibles of interest. Here are some of the key finding that we observed:
...
In term of the citizenship distribution, we observe the highest citizenship rate among all the Admission categories was for "Econonmic Immigrant, principal applicant" which was a 75% increase from 2001 to 2016. This claim is supported by the Poisson regression model, which indicates that individuals in this category are more likely to be granted citizenship compared to other categories. This is consistent with the findings in @tbl-table4, where we observe that the percentage of total count for "Economic Immigrants" were togeter 27.51% of the total count, which is the highest among all admission categories. 

Additionally, @fig-fig3 also showed that the median income for "Economic Immgirant" showed the biggest increase over the years, followed by "Immigrant sponsered by family". These results can be cross-verified with the Poisson regression model, in which positive coefficients for the admission categories were observed. 

This is a significant increase in the citizenship rate, suggesting that the Canadian government is successful in attracting
